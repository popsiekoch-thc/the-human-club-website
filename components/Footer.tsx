export default function Footer() {
  return (
    <footer style={{ background: 'var(--page-black)', color: 'var(--shell)', padding: '80px 40px 36px', borderTop: '1px solid var(--border)' }}>
      <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 'clamp(56px, 10vw, 156px)', lineHeight: 0.88, letterSpacing: '-0.04em', margin: '0 0 44px', color: 'var(--shell)' }}>
        Say&nbsp;hello,<br />
      ...
      >
        <div>
          <h4 style={{ fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', margin: '0 0 14px', color: 'rgba(225,225,213,0.5)' }}>
            — The Human Club
          </h4>
        ...
        <div>
          ...
          <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
            {[
              ...
              <li key={label}><a href={href} target="_blank" rel="noopener noreferrer" className="footer-link" style={{ fontFamily: 'var(--font-ui)', fontSize: 14, lineHeight: 1.6, color: 'var(--shell)' }}>{label}</a></li>
            ))}
          </ul>
        </div>
      </div>

      <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: 50, fontFamily: 'var(--font-ui)', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(225,225,213,0.45)' }}>
