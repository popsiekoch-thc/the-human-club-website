...
const MIXES: Mix[] = [
  ...
  },
  {
    id:    '2277289661',
    color: '30301c',
    title: 'Lemonella @ The Woods • Pandora Nexus 2026',
    host:  'Lemonella',
    scUrl: 'https://soundcloud.com/pandorafestival/lemonella-the-woods-o-pandora',
  },
]
...
function buildSrc(mix: Mix, autoplay: boolean): string {
  // Match the exact embed format SoundCloud provides:
  //   url=https%3A//api.soundcloud.com/tracks/soundcloud%253Atracks%253A<id>
  // The double-encoded `%253A` is intentional — SC's player decodes it once,
  // leaving the URN `soundcloud:tracks:<id>` for its API.
  ...
  return (
    <div>
      {/* Now-playing iframe — keyed on the mix id so it remounts cleanly on
          ...
          allowed by the browser autoplay policy because the page now has a
